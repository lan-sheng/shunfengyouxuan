        ...
        function T() {
            ...
            $(".lpleftbtn").click(function() {
                if (!li.is(":animated")) {
                    if (i >= n) {
                        i = 0;
                        li.eq(n).animate({ left: "-218px" }, speed);
...
function getAllPrice(id, css_id) {
    ...
    if (p_ids && e_ids) {
        $.post("/product/getAllPrice/actTag/1/", { pids: p_ids, eids: e_ids }, function(str) {
            if (str) {
                for (var i = 0; i < str.length; i++) {
                    //mprice 优选会员价  price 普通会员价  sfprice 商品价    str[i].presell.price  预售价
                    var e_id = str[i]['eid'];
                    var mPrice = '';
                    var member_rank = str[i].member_rank || '';
                    var member_rank_flag = '#' + member_rank;

                    if (typeof(str[i]['mprice']) !== 'undefined' && str[i]['mprice'] != str[i]['price'] && member_rank_flag.indexOf("20") != -1) {
                        mPrice = '<span><b>会员价</b><i>￥</i>' + str[i]['mprice'] + '</span>';
                    }
                    $("a[pid='" + str[i].pid + "']").attr("belong", str[i].belong);
                    if (typeof(str[i].presell) !== 'undefined') {
                        $("#" + css_id + e_id).html('<span><i>￥</i>' + str[i].presell.price + '</span>');
                        $("#cx_" + e_id).find(".gBtn").remove();
                    } else {
                        if (str[i].price != undefined && member_rank_flag.indexOf("10") != -1) {
                            $("#" + css_id + e_id).html('<span><i>￥</i>' + str[i]['price'] + '</span>' + mPrice); //价格渲染
                        } else {
                            $("#" + css_id + e_id).html('<span><i>￥</i>' + str[i]['sfprice'] + '</span>' + mPrice);
                        }
                    }
                }
            }
...
$(document).ready(function() {
    ...
    $('.app-android').attr('href', 'http://android.sfimg.cn/sfandroid');
    //右侧购物车
    sfAddCart.init();
    //右侧逛
    getHistory();
...
function isOnline(home_url, passport_url) {
    $.getJSON("/ajax/isOnline/?callback=?", function(data) {
        var wechatSrc = 'http://chat.voicecom.cn/weboperator/Operator/banner.aspx?sid=1&MemberID=' + data.memberId;
        $('#wechat_attr').attr('src', wechatSrc);
        if (data.welcome) {
            var wechatUrl = "http://webchat.t.com:8000/main.asp?sid=1&sTag=T&style=0&did=1&MemberID=" + data.memberId;
            ...
            $('#side_kefu').html('<a href="' + wechatUrl + '" target="_blank" class="s-kefu">客服</a>');
        } else {
            ...
            var welComeMsg = '';
            welComeMsg = '嘿，欢迎来顺丰优选！';
            var loginHtml = '<a href="' + passport_url + '/reg/?returnUrl=' + escape(document.location.href) + '">免费注册</a>';
            if (data.RegActiveName) {
                loginHtml = '<div class="newregbox"><a href="' + passport_url + '/reg/?returnUrl=' + escape(document.location.href) + '">免费注册</a><div class="newregdiv ac_newregdiv"><span class="up-icon"></span><span class="newregTips"><em class="newwords">' + data.RegActiveName + '</em></span></div></div>';
            }
            $('#login').html(welComeMsg + '<a href="' + passport_url + '/?returnUrl=' + escape(document.location.href) + '">请登录</a> | ' + loginHtml);
            $('#side_kefu').html('<a href="' + passport_url + '/?returnUrl=' + escape(document.location.href) + '" class="s-kefu">客服</a>');
        }
        if (data.qqcb) {
        ...
        if ((str.nowList.length < 1 && str.nextList.length > 0)) {
            ...
            for (i = 0; i < 5; i++) {
                ...
                if (temp) {
                    ...
                    if (temp.isStork) {
                        var priceHtml = '';
                        if (temp.isrank == 1 || temp.allrank == 20) {
                            priceHtml = '会员价 <b>￥' + temp.mgprice + '</b>';
                        }
                        $nowCon += '<li id="cx_q_' + temp.product_id + '" eid="q_' + temp.product_id + '" goods="' + temp.product_id + '">' +
                            '<div class="pImg">' +
                            '<a title="' + temp.title + '" target="_blank" href="' + temp.url + trackref + '">' +
                            '<img onerror="this.src=\'http://i.sfimg.cn/html/images/150pic.jpg\'" alt="' + temp.title + '" src="' + temp.img + '">' +
                            '</a>' +
                            '</div>' +
                            '<div class="title-a">' +
                            '<a title="' + temp.title + '" target="_blank" href="' + temp.url + trackref + '" >' + temp.title + '</a>' +
                            '</div>' +
                            '<div class="pt-price clearfix">' +
                            '<div class="fl p1">￥' + temp.qgprice + '</div>' +
                            '</div>' +
                            '<div class="price">' +
                            priceHtml +
                            '</div>' +
                            '<a class="rushBuy p_qg_btn" href="javascript:void(0)" data_url="' + temp.img + '" belong="' + temp.belong + '" pid="' + temp.product_id + '" >抢购</a>';
                    } else {
                        $nowCon += '<li class="tbover">' +
                            '<div>' +
                            '<a title="' + temp.title + '" target="_blank" href="' + temp.url + trackref + '">' +
                            '<img onerror="this.src=\'http://i.sfimg.cn/html/images/150pic.jpg\'" alt="' + temp.title + '" src="' + temp.img + '"/>' +
                            '<span class="yover"></span>' +
                            '</a>' +
                            '</div>' +
                            '<div class="title-a">' +
                            '<a title="' + temp.title + '" target="_blank" href="' + temp.url + trackref + '">' + temp.title + '</a>' +
                    ...
                    $nowCon += '</li>';
                    if ($timeClock == "") {
                        $timeClock = '<script>timechange("now",' + temp.stopTime + ');</script>';
                    }
                } else {
                    $nowCon += '<li class="beginning">';
                    $nowCon += '<div class="pImg"><img src="http://i.sfimg.cn/html/images/blank.gif" alt="" width="164" class="lazy"/></div>';
        ...
        if (str.nextList.length < 1) {
            ...
            for (i = 0; i < 5; i++) {
                ...
                if (temp) {
                    styleId = temp.aid;
                    $nextCon += "<li>";
                    $nextCon += '<div class="pImg"><a title="' + temp.title + '" target="_blank" href="' + temp.url + trackref + '"><img onerror="this.src=\'http://i.sfimg.cn/html/images/150pic.jpg\'" alt="' + temp.title + '" src="' + temp.img + '"></a></div>';
                    $nextCon += '<div class="title-a"><a title="' + temp.title + '" target="_blank" href="' + temp.url + trackref + '">' + temp.title + '</a></div>';
                    var qgprice = '';
                    if (temp.allrank != 20) {
                        qgprice = '<div class="fl p1">￥' + temp.qgprice + '</div>';
                    }
                    $nextCon += '<div class="pt-price clearfix">' + qgprice + '<div class="fr p2">￥' + temp.sfprice + '</div></div>';
                    if (temp.isrank == 1 || temp.allrank == 20) {
                        $nextCon += '<div class="price">会员价 <b>￥' + temp.mgprice + '</b></div>';
                    } else {
                        $nextCon += '<div class="price"></div>';
                    }
                    $nextCon += '<a class="rushBuy p_qg_btn" target="_blank" href="' + temp.url + trackref + '">看看</a>';
                    $nextCon += '</li>';
                    if ($nextClock == "") {
                        $nextClock = '<script>timechange("next",' + temp.startTime + ');</script>';
                    }
                } else {
                    $nextCon += '<li class="beginning">';
                    $nextCon += '<div class="pImg"><img src="http://i.sfimg.cn/html/images/blank.gif" alt="" width="164" class="lazy"/></div>';
    ...
    $input.select().autocomplete(url, {
        ...
        formatItem: function(row) { // 解析数据
            ...
            if (type == 1 && row.length > 1) { // 搜索商品不带统计数据
                ...
                //alert(row[4])
                label = row[1], categoryId = row[2];
                categoryName = row[3];
                categoryTwoId = row[4];
                categoryTwoName = row[5];
                count = parseInt(row[6]);
                ...
                var searchkeyword = row[1];
                var searchkeywordcount = row[2];
...
$(document).ready(function() {
    ...
    var getFootRegionSf = function() {
        var host_www2 = "http://www." + window.location.host.split(".")[1] + '.' + window.location.host.split(".")[2];
...
var getCookie = function(searchName) {
    ...
    for (var i = 0; i < cookies.length; i++) {
        ...
        var cookieName = $.trim(cookiesCrumbs[0]);
        var cookieValue = $.trim(cookiesCrumbs[1]);
...
var subfootedm = function() {
    ...
    $.getJSON(web, { "boxlist": 1, "mail": mail, format: "json" }, function(data) {
        switch (data.type) {
            ...
            case -2:
                $("#emailMsg").html(data.msg);
                $("#emailMsg").attr("class", "ydy");
                $("#emailMsg").show();
                break;
            case 1:
                location.href = host + "/edm/index/";
        }
    });
}
...
$(function() {
    ...
    if ($(".floatBar").length > 0) {
        var floatBar = function() {
            var floatBarTop = $(document).scrollTop(),
                startTop = $(".navmenu").offset().top;
    ...
    if (index_win_w <= 1440) { $(".index_rfloat").addClass("index_side"); } else { $(".index_rfloat").removeClass("index_side"); }
    $(window).resize(function() {
        var index_win_width = $(window).width();
        if (index_win_width <= 1440) { $(".index_rfloat").addClass("index_side"); } else { $(".index_rfloat").removeClass("index_side"); }
    });
})
